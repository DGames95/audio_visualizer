...
# Read a WAV file
sample_rate, data = wavfile.read("your_audio_file_cut.wav")
if len(data.shape) > 1:
...
# init array storing the previous frame that will be mixed in with the new one to make it less erratic
...
for start in range(0, len(data) - fft_size, step_size):
    ...
    # Mix in the previous frame with percentage alpha
    alpha = 0.2
    ...
    for i, energy in enumerate(bin_energies):
        ...
        # insert colors here RGB
        colors = (intensity, (150-intensity/2), bintensity)

        cv2.rectangle(frame, (i, 100), (i, 100 - int(energy * 100)), colors, -1)
